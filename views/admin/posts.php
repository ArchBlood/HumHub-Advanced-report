...
<div class="container-fluid">
    ...
    <div class="panel panel-default">
        <div class="panel-heading"><strong>Posts</strong> reported</div>
        <div class="panel-body">

        <?php if (empty($reports)) : ?>
            <br/> <br/>
            <?= Yii::t('ReportModule.widgets_views_reportContentAdminGrid', 'There are no reported posts.') ?>
            <br/> <br/>
        <?php else : ?>
        ...
        <?= GridView::widget([
            ...
            'columns' => [
                ...
                [
                    ...
                    'format' => 'raw',
                    'value' => function($report) {
                        $result = Html::tag('p',  RichText::preview($report->post->getContentDescription(), 60));
                        $userLink = Html::a(Html::encode($report->post->createdBy->displayName), $report->post->createdBy->getUrl());
                        $displayNameLink = Yii::t('ReportModule.base', 'created by :displayName',  [':displayName' => $userLink]);
                        $result .= Html::tag('small', $displayNameLink .' '. TimeAgo::widget(['timestamp' => $report->post->created_at]), ['class' => 'media']);
                        return $result;
                    }
                ],
                [
                ...
                [
                    ...
                    'value' => function($report) {
                        $approve = ModalConfirm::widget([
                            ...
                            'linkHref' => Url::to(["//report/report/appropriate", 'id' => $report->id, 'resource' => 'post']),
                        ]);

                        $review =  Html::a('<i aria-hidden="true" class="fa fa-eye"></i>', $report->post->content->getUrl(), [
